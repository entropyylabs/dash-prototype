...
module.exports = async (app) => {
  const API_KEY = process.env.NFTPORT_API_KEY;
  ...
  app.retrieveDeployedContract = async (txHash) => {
    const response = await axios.get(
      `https://api.nftport.xyz/v0/contracts/${txHash}?chain=polygon`,
      {
        headers: {
          "content-type": "application/json",
          Authorization: API_KEY,
  ...
  app.mintNFT = async (contract_address, metadata_uri, mint_to_address) => {
    const response = await axios.post(
